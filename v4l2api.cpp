...
void V4l2Api::run()
{
    ...
    while(1)
    {
        ...
        yuyv_to_rgb888(reinterpret_cast<unsigned char*>(buffer), reinterpret_cast<unsigned char*>(rgbbuffer));
        //把RGB数据转为QImage
        QImage image(reinterpret_cast<uchar*>(rgbbuffer), WIDTH, HEIGHT, QImage::Format_RGB888);
